...
const vault_controller_1 = __webpack_require__(45);
const vault_service_1 = __webpack_require__(46);
const telegram_service_1 = __webpack_require__(43);
const qwen_service_1 = __webpack_require__(44);
...
exports.AppModule = AppModule = __decorate([
    (0, common_1.Module)({
        ...
        controllers: [market_controller_1.MarketController, ens_controller_1.EnsController, oracle_controller_1.OracleController, swap_controller_1.SwapController, gas_station_controller_1.GasStationController, mt5_controller_1.Mt5Controller, mm_controller_1.MarketMakerController, agents_controller_1.AgentsController, freeze_controller_1.FreezeController, admin_controller_1.AdminController, vault_controller_1.VaultController],
        providers: [admin_service_1.AdminService, freeze_service_1.FreezeService, telegram_service_1.TelegramService, qwen_service_1.QwenService, vault_service_1.VaultService,
...
__decorate([
    ...
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], AgentsController.prototype, "trades", null);
...
let AdminService = AdminService_1 = class AdminService {
    ...
    async monitor() {
        ...
        try {
            events.push(...(await this.checkGasStation()));
        }
        catch (e) {
            events.push(`GasStation check error: ${e.message}`);
        }
        if (events.length > 0) {
    ...
    async checkOracle() {
        ...
        try {
            ...
            await client.readContract({ address: oracle, abi, functionName: "pairsCount" });
            // all good — no event (silence in normal state)
        }
        catch (e) {
    ...
    // 4. GasStation security: fee sane + relayer set (vulnerability watch)
    async checkGasStation() {
        const events = [];
        const gasStation = process.env.GAS_STATION_ADDRESS || "0x81337bfa2c9d01d6543e91a7641258eeceff7cfe";
        const client = (0, chains_1.getClient)(1); // GasStation is on mainnet
        if (!client)
            return events;
        try {
            const abi = (0, viem_1.parseAbi)([
            ...
            const satlPerSwap = await client.readContract({ address: gasStation, abi, functionName: "satlPerSwap" });
            const relayer = await client.readContract({ address: gasStation, abi, functionName: "relayer" });
            if (satlPerSwap > 10n * 10n ** 18n) {
                events.push(`GasStation WARNING: fee = ${Number(satlPerSwap) / 1e18} SATL (suspiciously high)`);
            }
            if (relayer === "0x0000000000000000000000000000000000000000") {
                events.push("GasStation WARNING: relayer is zero address");
            }
        }
        catch (e) {
            events.push(`GasStation unreachable: ${e.shortMessage || e.message}`);
        }
        return events;
    }
...
let TelegramService = TelegramService_1 = class TelegramService {
    ...
    // Send alerts to the owner (uses discovered chatId)
    async sendMessage(text) {
        if (!this.chatId)
            return false;
        return this.sendToChat(this.chatId, text);
    }
    // Poll for owner commands (every 30s) — discovers chatId AND processes commands in one pass
    async pollCommands() {
        if (!this.token)
            return;
        try {
            ...
            for (const u of updates) {
                ...
                if (chatId)
                    this.chatId = chatId; // discover owner chat_id
                if (!msg.text)
                    continue;
    ...
    // Generate a report using Qwen LLM + live exchange state
    async generateReport() {
        const freezes = await this.prisma.marketFreeze.findMany({
            where: { status: { in: ["frozen", "active"] } },
            include: { contributions: true },
        }).catch(() => []);
        const markets = await this.prisma.market.findMany({ where: { status: "ACTIVE" } }).catch(() => []);
        const freezeSummary = freezes.map((f) => {
            const committed = f.contributions.filter((c) => c.committed).length;
            const paid = f.contributions.filter((c) => c.paid).length;
            return `- ${f.marketId}: сумма $${f.amountUsd}, фаза ${f.phase}, участников ${committed}, оплатили ${paid}`;
        }).join("\n") || "нет активных заморозок";
        const raw = `Текущее состояние биржи:\n` +
            `Активных рынков: ${markets.length}\n` +
            `Заморозки:\n${freezeSummary}`;
        const ai = await this.qwen.generateReport(raw);
        return `📊 MirrorBook — сводка\n\n${ai || raw}`;
    }
};
...
exports.TelegramService = TelegramService = TelegramService_1 = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [typeof (_a = typeof qwen_service_1.QwenService !== "undefined" && qwen_service_1.QwenService) === "function" ? _a : Object, typeof (_b = typeof prisma_service_1.PrismaService !== "undefined" && prisma_service_1.PrismaService) === "function" ? _b : Object])
], TelegramService);
...
let QwenService = QwenService_1 = class QwenService {
    ...
    model = process.env.QWEN_MODEL || "deepseek/deepseek-v4-flash";
        ...
        try {
            const r = await fetch(`${this.baseUrl}/chat/completions`, {
                ...
                body: JSON.stringify({
                    model: this.model,
                    messages: [
                        { role: "system", content: "Ты — AI-администратор биржи MirrorBook. Отвечай владельцу кратко и по делу, на русском языке." },
            ...
            const data = await r.json();
            if (data?.error) {
                this.logger.warn(`Qwen API error: ${JSON.stringify(data.error).slice(0, 200)}`);
                return "";
            }
            return data?.choices?.[0]?.message?.content || "";
        }
        catch (e) {
            this.logger.warn(`Qwen API call failed: ${e.message}`);
            return "";
        }
    }
};
...
let VaultController = class VaultController {
    ...
    async info() {
        return this.vaultService.getInfo();
    }
    async balances(user) {
        return this.vaultService.getBalances(user);
    }
    encodeDeposit(body) {
        return this.vaultService.encodeDeposit(body.token, body.amount);
    }
    encodeWithdraw(body) {
        return this.vaultService.encodeWithdraw(body.token, body.amount);
    }
    encodeTrade(body) {
        return this.vaultService.encodeTrade(body.user, body.tokenIn, body.tokenOut, body.fee, body.amountIn, body.minAmountOut);
    }
};
...
__decorate([
    ...
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], VaultController.prototype, "info", null);
__decorate([
    (0, common_1.Get)("balances/:user"),
    __param(0, (0, common_1.Param)("user")),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], VaultController.prototype, "balances", null);
__decorate([
    (0, common_1.Post)("encode-deposit"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], VaultController.prototype, "encodeDeposit", null);
__decorate([
    (0, common_1.Post)("encode-withdraw"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], VaultController.prototype, "encodeWithdraw", null);
__decorate([
    (0, common_1.Post)("encode-trade"),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], VaultController.prototype, "encodeTrade", null);
exports.VaultController = VaultController = __decorate([
    (0, common_1.Controller)("api/v1/vault"),
    __metadata("design:paramtypes", [typeof (_a = typeof vault_service_1.VaultService !== "undefined" && vault_service_1.VaultService) === "function" ? _a : Object])
], VaultController);
...
const SATL = "0x852E896161931bB9f785bF75365927e70f47B988"; // WSATL on Base
...
const ABI = (0, viem_1.parseAbi)([
    ...
    "function trade(address user, address tokenIn, address tokenOut, uint24 fee, uint256 amountIn, uint256 minAmountOut)",
...
let VaultService = VaultService_1 = class VaultService {
    ...
    constructor() {
        ...
        this.client = (0, viem_1.createPublicClient)({ chain: chains_1.base, transport: (0, viem_1.http)(rpc) });
    }
    async getInfo() {
        try {
            const agent = await this.client.readContract({ address: VAULT, abi: ABI, functionName: "agent" });
            const paused = await this.client.readContract({ address: VAULT, abi: ABI, functionName: "paused" });
            return { vault: VAULT, chainId: 8453, agent, paused, satl: SATL, weth: WETH };
        }
        catch (e) {
            this.logger.warn(`vault info error: ${e.shortMessage || e.message}`);
            return { vault: VAULT, chainId: 8453, agent: null, paused: null, satl: SATL, weth: WETH };
        }
    }
    async getBalances(user) {
        try {
            const satlBal = await this.client.readContract({ address: VAULT, abi: ABI, functionName: "balances", args: [user, SATL] });
            const wethBal = await this.client.readContract({ address: VAULT, abi: ABI, functionName: "balances", args: [user, WETH] });
            return {
                user,
                satl: satlBal.toString(),
                weth: wethBal.toString(),
            };
        }
        catch (e) {
            this.logger.warn(`vault balances error: ${e.shortMessage || e.message}`);
            return { user, satl: "0", weth: "0" };
        }
    }
    encodeDeposit(token, amount) {
        const data = (0, viem_1.encodeFunctionData)({ abi: ABI, functionName: "deposit", args: [token, BigInt(amount)] });
        return { to: VAULT, data };
    }
    encodeWithdraw(token, amount) {
        const data = (0, viem_1.encodeFunctionData)({ abi: ABI, functionName: "withdraw", args: [token, BigInt(amount)] });
        return { to: VAULT, data };
    }
    encodeTrade(user, tokenIn, tokenOut, fee, amountIn, minAmountOut) {
        const data = (0, viem_1.encodeFunctionData)({
            abi: ABI, functionName: "trade",
            args: [user, tokenIn, tokenOut, fee, BigInt(amountIn), BigInt(minAmountOut)],
        });
        return { to: VAULT, data };
    }
};
